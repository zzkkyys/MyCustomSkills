...
字体字号只是格式的一半，另一半是**正文和它引用的东西对不对得上**。这四项 `scripts/audit.py` 会报前三项，第四项要人工看：

1. **孤图**——图片插进去了，正文却没有"如图 N 所示"一类交代。评审会问这张图在说明什么。
2. **重复图题**——用 `![图N 标题](路径)` 插图后，又另起一行写了一遍图题。pandoc 会从 alt text 生成题注，转出来是两个图题。二选一。
3. **引文编号不按首次出现顺序**——正文第二句就出现 `[17-18]`。若所属体例要求按引用顺序编号，定稿前重排文献表。
4. **正文与公式/图不符**——正文说模型加入了"总残差、三类关系残差及其时间分布特征"，公式里却只有一个符号；或正文描述的流程与图里箭头方向不一致。**每个公式回到引用它的那句话，逐个符号对一遍。**

```bash
python3 scripts/audit.py 正文目录/ --skip 00-      # 前三项
```
