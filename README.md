...
| Skill | 用途 |
|---|---|
| [paper-revision-review](paper-revision-review/) | 把修改建议做成可交互的**离线审阅网页**：改动处内联标记（新增/删除/改写/结构调整），点击弹出原文／改后／**为什么改**；支持标记与终稿视图切换、按严重程度筛选、逐条浏览、勾选已处理。含模板、渲染脚本与五重校验器，产物为单文件零依赖 HTML。附**分章审阅清单**（摘要引言/相关工作/理论/方法/实验/结论各自的高频问题）与**第二意见协作流程**（让另一模型独立复审再三方对照，含「核实后再采信」纪律）。不限语言与学科 |
